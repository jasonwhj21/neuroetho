...
for i = 1:expnum
    ...
    for j = 1:5:rows-1
        ...
        dal_pos2 = [dal_positions_x(j + 5), dal_positions_y(j+5)];
        
        other_pos1 = [other_positions_x(j), other_positions_y(j)];
        other_pos2 = [other_positions_x(j + 5), other_positions_y(j+5)];

        dal_disp = dal_pos2 - dal_pos1; 
        other_disp = other_pos2 - other_pos1;
    
        time_elapsed = time_col(j+5, 1) - time_col(j, 1);
        ...
        dal_velocities((j+4)/5,:) = dal_velocity; %Store velocities temporarily
        other_velocities((j+4)/5,:) = other_velocity;
        
    end
    for n = 1:rows-2
        if all(~isnan(dal_velocities(n,:))) && all(~isnan(dal_velocities(n+1,:))) && all(~isnan(other_velocities(n,:))) && all(~isnan(other_velocities(n+1,:)))
            dal_angular_speed = subspace(dal_velocities(n,:)',dal_velocities(n+1,:)')/(frame2time*5); 
            other_angular_speed = subspace(other_velocities(n,:)',other_velocities(n+1,:)')/(frame2time*5);
            dal_curvatures(n,1) = dal_angular_speed;
            other_curvatures(n,1) = other_angular_speed; %Angular speeds are in units of radians per second
        end
    end
