...
for i = 1:expnum
    ...
    for j = 1:size(dal_velocities,1)-1
        ...
        dotted_other = dot(other_velocities(j,:), other_velocities(j+1,:))/(vecnorm(other_velocities(j,:))* vecnorm(other_velocities(j+1,:)));
        other_angular_speed = acos(dotted_other)/(frame2time*5); 
    ...
    rawDatafiltcurvature{i,1} = table(time_col(1:size(dal_curvatures)), dal_curvatures, other_curvatures); %Finally, experiment i is complete and the cell is updated.
end